...
let cancellable = function(fn , args , t , cancelT) {
    let storeTimeInterval = [];
   let checkInterval = setInterval(() => {
        let calculation = fn(args)
        //storing the array
        storeTimeInterval[storeTimeInterval.length] = { "time" : t , "returned" : calculation }
        t += 35;
        storeTimeInterval.forEach((item) => {if(item.time > cancelT) {clearTimeout(checkInterval)}} )//check the condition of cancelT
        console.log("output--->",storeTimeInterval)        
    },t)
}
args = [4]
t = 0
cancelT = 190
let result = cancellable(fn , args , t , cancelT)
